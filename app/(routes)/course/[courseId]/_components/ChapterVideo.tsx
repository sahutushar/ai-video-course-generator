...
const SlideIFrameWithReveal = ({ slide, duration }: { slide: Slide; duration: number }) => {
    ...
    const revealPlan = useMemo(() => {
        ...
        // Evenly distribute reveals across the slide duration
        const interval = duration / (ids.length + 1);
        return ids.map((id, i) => ({
            id,
            at: interval * (i + 1),
        }));
    }, [slide.revelData, duration]);
        ...
        <AbsoluteFill style={{ backgroundColor: "#000" }}>
            {timeline.map(({ slide, from, dur }) => (
                <Sequence key={from} from={from} durationInFrames={dur}>
                    <SlideIFrameWithReveal slide={slide} duration={dur / fps} />
                </Sequence>
            ))}
        </AbsoluteFill>
    );
